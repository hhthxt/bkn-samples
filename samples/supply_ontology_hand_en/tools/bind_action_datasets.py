...
def run_bind(config, mapping, *, dry_run, run_cmd=run_cmd):
    ...
    for binding in bindings:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False, encoding="utf-8") as tmp:
            ...
            body_path = tmp.name
        try:
            run_cmd(["openbkn", "--json", "bkn", "object-type", "update", kn_id, binding["object_type_id"], "--body-file", body_path])
        finally:
...
def main():
    ...
    print(json.dumps(run_bind(config, payload, dry_run=not args.apply), ensure_ascii=False, indent=2))
