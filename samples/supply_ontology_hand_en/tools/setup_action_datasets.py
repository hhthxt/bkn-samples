...
EXPECTED_TABLES = {"sc_pr_decision", "sc_plan_monitor_task", "sc_plan_monitor_item"}
...
def verify_tables(connection, schema: str = "public") -> dict:
    query = "SELECT table_name FROM information_schema.tables WHERE table_schema = %s AND table_name = ANY(%s)"
...
def main():
    ...
    try:
        import json
        print(json.dumps(apply_ddl(connection, schema=args.schema), ensure_ascii=False, indent=2))
    finally:
        connection.close()
