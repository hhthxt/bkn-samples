...
- 场景能力设计（扩展阅读）：`docs/agent-scenario-kn-capability-design_cn.md` · [English](docs/agent-scenario-kn-capability-design.md)
...
在 Agent / 对话界面中，使用固定问法验证知识网络是否「可用、可答、可联查」。

**测试记录表（打印后人工勾选「通过」列）：**

| # | 问法 | 期望要点 | 实际 | 通过 |
|---|------|----------|------|------|
| Q1 | 这个知识网络里有多少种产品？ | 能给出产品数量或列表，不报未绑定 | | ☐ |
| Q2 | 查产品 `382-000005` 的基本信息 | 返回名称/编码 | | ☐ |
| Q3 | 销售订单里产品 `382-000005` 有哪些订单？ | 能联到 SO，不报无关系 | | ☐ |
| Q4 | 产品 `382-000005` 的 BOM 用到哪些物料？ | 返回物料列表 | | ☐ |
| Q5 | 对产品 `382-000005` 做齐套/倒排诊断，需求日 `2026-05-14` | P0：不因无数据失败；P1：含交期/缺口要点 | | ☐ |

> 样例产品码 `382-000005` 取自 `data/hd_product_view.csv` 首行；需求日 `2026-05-14` 对应该产品在样例销售订单中的一条承诺交期。可按现场数据替换，但建议固定一组便于回归对比。

**判定原则：**

- P0：至少 3 条问法（Q1～Q4 中任选）不因「无数据 / 未绑定」失败
- P1：Q5 齐套/倒排类问法能产出含交期 / 缺口要点的回答（见场景设计文档 §3）
