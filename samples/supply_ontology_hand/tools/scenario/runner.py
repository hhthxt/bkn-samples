...
from fn import DEFAULT_BUSINESS_DATE, backward_plan, shared_contention, total_sellable
...
class FulfillmentCommitmentRunner:
    ...
    def run(self, *, product: str, forecast_id: str, demand_end: str, demand_qty: float, business_date: str = DEFAULT_BUSINESS_DATE, substitute_enabled: bool, demands: list[dict[str, Any]] | None = None, report_grain: str = "summary") -> dict[str, Any]:
        if not forecast_id:
        ...
        s1 = backward_plan(envelope.snapshot, product, forecast_id=forecast_id, demand_end=demand_end, demand_qty=demand_qty, business_date=business_date, substitute_enabled=substitute_enabled, report_grain=report_grain)
