...
def test_build_bindings_qualifies_schema_and_uses_kn_id():
    mapping = {
        "bindings": [
            {"object_type_id": "supply_ontology_hand_mon_task", "dataset": "sc_plan_monitor_task"}
        ]
    }
    assert build_bindings(mapping, kn_id="supply_ontology_hand", schema="public") == [
        {
            "kn_id": "supply_ontology_hand",
            "object_type_id": "supply_ontology_hand_mon_task",
            "dataset": "public.sc_plan_monitor_task",
        }
    ]
...
def test_build_bindings_expands_schema_placeholder():
    ...
    assert build_bindings(mapping, kn_id="supply_ontology_hand", schema="public")[0]["dataset"] == "public.sc_pr_decision"
...
def test_dry_run_does_not_update_object_type():
    ...
    report = run_bind(
        {"openbkn": {"kn_id": "supply_ontology_hand"}, "database": {"schema": "public"}},
...
def test_apply_resolves_catalog_resource_and_uses_resource_data_source():
    ...
    def run_cmd(args):
        ...
        if args[2:5] == ["bkn", "object-type", "get"]:
            return '{"entries":[{"id":"ot1","name":"监控任务","data_properties":[]}]}'
        return "{}"

    report = run_bind(
        {"openbkn": {"kn_id": "supply_ontology_hand"}, "vega": {"catalog_id": "catalog1"}, "database": {"schema": "public"}},
        {"bindings": [{"object_type_id": "ot1", "dataset": "sc_plan_monitor_task"}]},
        dry_run=False,
        run_cmd=run_cmd,
    )
