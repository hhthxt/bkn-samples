...
def _resolved_context(params):
    context_payload = params.get("resolved_context")
    if context_payload is not None:
        return context_payload
    compressed = params.get("resolved_context_compressed")
    ...
    try:
        ...
        raise ValueError("resolved_context_compressed 不是合法 zlib+base64 JSON") from exc
...
def execute(event):
    ...
    context_payload = _resolved_context(params)
    ...
    if operation == "bom_list":
        ...
        result = open_forecast_count(context_payload.get("rows", {}).get("forecast") or [], product_code=params.get("product_code"))
        ...
        raise ValueError("不支持的 operation: %s" % operation)
